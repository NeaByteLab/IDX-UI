...
export function useSecurities(params?: Types.SecuritiesParams): Types.UseSecuritiesResult {
  ...
  useEffect(() => {
    ...
    setError(null)
    const query = {
      ...API.buildPaginationQuery(params),
      ...(params?.code && { code: params.code }),
      ...(params?.board && { board: params.board })
    }
    API.apiGet<Types.PaginatedResponse<unknown>>(API.securitiesPath(), query)
      .then((res) => {
  ...
  }, [refetchKey, params?.limit, params?.offset, params?.includeTotal, params?.code, params?.board])
