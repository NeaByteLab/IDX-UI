...
export function useCompanyFinancialReports(
  code: string | null,
  params?: Types.PaginationParams
): Types.UseCompanyFinancialReportsResult {
  ...
  useEffect(() => {
    ...
    API.apiGet<Types.PaginatedResponse<unknown>>(API.companyFinancialReportsPath(code), query)
      .then((res) => {
  ...
  }, [code, refetchKey, params?.limit, params?.offset, params?.includeTotal])
