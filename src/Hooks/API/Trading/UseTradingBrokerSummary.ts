...
export function useTradingBrokerSummary(
  date: string | null,
  params?: Types.PaginationParams
): Types.UseTradingBrokerSummaryResult {
  ...
  useEffect(() => {
    ...
    const dateParam = dateToYYYYMMDD(date)
    if (!dateParam) {
      setData(null)
      setMeta(null)
      setLoading(false)
    ...
    API.apiGet<Types.PaginatedResponse<unknown>>(API.tradingBrokerSummaryPath(), query)
      .then((res) => {
