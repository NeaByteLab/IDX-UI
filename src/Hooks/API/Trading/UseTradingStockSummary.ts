...
export function useTradingStockSummary(
  date: string | null,
  params?: Types.PaginationParams
): Types.UseTradingStockSummaryResult {
  ...
  useEffect(() => {
    ...
    const dateParam = Utils.dateToYYYYMMDD(date)
    if (!dateParam) {
      setData(null)
      setMeta(null)
      setLoading(false)
    ...
    API.apiGet<Types.PaginatedResponse<unknown>>(API.tradingStockSummaryPath(), query)
      .then((res) => {
  ...
  }, [date, refetchKey, params?.limit, params?.offset, params?.includeTotal])
