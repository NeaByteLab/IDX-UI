...
export function useMarketIndexChart(
  code: string | null,
  params?: Types.UseMarketIndexChartParams
): Types.UseMarketIndexChartResult {
  ...
  useEffect(() => {
    ...
    API.apiGet<Types.PaginatedResponse<unknown>>(API.marketIndexChartPath(code), query)
      .then((res) => {
  ...
  }, [code, refetchKey, params?.limit, params?.offset, params?.includeTotal, params?.period])
